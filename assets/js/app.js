...
function tzController($scope, $http) {
	$scope.db;
	$http.get('assets/fixtures/cities.json').success(function(response) {
		$scope.db = (response);
		init();
	});
	$scope.homeCity = {};
	...
	var initUi = function() {
		...
		angular.forEach($scope.myPlaces, function(myPlace) {
		    ...
		    console.log($scope.db.myPlace.toString(format, tz);
		    console.log("sparta");
		...
		var myScroll = new iScroll('added-timezone-blocks', {
